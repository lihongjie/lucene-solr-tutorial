...
!function (e, t) {
    ...
    function r(r, u) {
        ...
        function s(e) {
            var t = T.map(o), n = new RegExp("\\b((" + t.join("|") + ")[\\S']*)", "gi");
        ...
        function p(a) {
            ...
            var l, u;
            if (null !== k) {
                if (a && (k = c(k, T)), !k.length)return void(L.style.display = "none");
                ...
                var p, v, d = null, f = [];
                k.length || (N.value = "");
                var m = T.map(o), g = new RegExp("\\b(" + m.join("|") + ")(\\w+)\\b", "gi"), h = t.createDocumentFragment();
                for (l = 0, u = k.length; u > l; l++) {
                    for (var y, E; null !== (y = g.exec(k[l][1]));)E = new RegExp("\\b" + o(y[0]) + "\\b", "gi"), d = y[y.length - 1], void 0 === d || E.test(r.value) || w === l && f.push(d);
                    p = n("p"), l === w && x && p.classList.add("selected"), p.dataset.i = l, v = n("a", {
                        innerHTML: s(k[l][1]),
                        href: k[l][0]
                    }), p.appendChild(v), h.appendChild(p), b.push(p)
                }
                L.appendChild(h), f.length && " " !== r.value.charAt(r.value.length - 1) ? (d = f[w % f.length], N.value = r.value + d) : N.value = ""
            }
        }
        ...
        function d(t) {
            var n, a;
            if ("tab" === t) N.value !== r.value && (r.value = N.value + " "), r.value !== N.value && g(), x = !0; else if ("down" === t || "up" === t) {
                ...
                var u = b[w], o = u.getElementsByTagName("a")[0];
        ...
        function f(e) {
            var t = {13: "enter", 9: "tab", 38: "up", 40: "down", 27: "esc"};
        ...
        function g() {
            ...
            if (N.value.length && (-1 === N.value.indexOf(r.value.trim()) && (N.value = r.value), "block" === L.style.display && (T = r.value.trim().split(/\s+/), X = e.setTimeout(function () {
                    p(!0)
                }, 150))), w = 0, x = !1, j[r.value.trim()]) {
                var t = j[r.value.trim()];
                T = t.terms, k = t.results, p()
            } else {
                if (R && R.abort(), e.XMLHttpRequest) R = new e.XMLHttpRequest; else {
                ...
                if (r.value.trim().length) {
                    ...
                    if (j[n] && !j[n].results.length)return void(j[r.value.trim()] = {results: []})
                }
                R.onreadystatechange = function () {
                    if (4 === R.readyState)if (200 === R.status) {
                        if (M === r.value) {
                            var e = JSON.parse(R.responseText);
                            j[r.value.trim()] = e, T = e.terms, k = e.results, p()
                        }
                    } else 0 !== R.status && (console.warn(R.status, R.responseText), m())
                }, R.open("GET", u.url + encodeURIComponent(r.value.trim()), !0), M = r.value, R.send()
            }
        }
        ...
        var E = n("span", {className: "_ac-wrap"}), N = n("input", {
            tabindex: -1,
            spellcheck: !1,
            autocomplete: "off",
            readonly: "readonly",
